...
for seed in seeds:
    random.seed(seed)
    np.random.seed(seed)
    dump_folder = '../../stage2_processed_data/LDKP3km-lr-1000/{}/v6_bt_aug/'.format(seed)
    Path(dump_folder).mkdir(parents=True, exist_ok=True)

    process_data('LDKP3km-lr-1000_train.jsonl'   , dump_folder, seed)
    process_data('LDKP3km-lr-1000_dev_LDKP3km-lr-1000.jsonl' , dump_folder, seed)
    process_data('LDKP3km-lr-1000_test_LDKP3km-lr-1000.jsonl', dump_folder, seed)
    shutil.copy('../../processed_data/{}/aug_v6/LDKP3km-lr-1000_metadata.pkl'.format(seed), dump_folder)  # Copy metadata file as it is