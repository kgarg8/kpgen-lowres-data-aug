...
res = pickle.load(open('../../processed_data/LDKP10km/LDKP10km_metadata.pkl', 'rb')) # since metadata file is common
...
def process_data(file, path):
    ...
    for i in tqdm(range(len(data))):
        ...
        if 'train' in file:
            new_src, new_src_vec, new_trg, new_trg_vec = keyphrase_dropout_fn(src, data[i]['trg'], vocab2idx)
            srcs.append(new_src)
    ...
    jsonl_save(filepath=path+file.replace('LDKP10km', 'LDKP10km-lr-8000'), data_dict=data_dict)
...
for seed in seeds:
    random.seed(seed)
    np.random.seed(seed)
    dump_folder = '../../stage2_processed_data/LDKP10km-lr-8000/{}/v2_kpdrop_aug/'.format(seed)
    ...
    os.rename(dump_folder+'LDKP10km_metadata.pkl', dump_folder+'LDKP10km-lr-8000_metadata.pkl')