...
for seed in seeds:
    random.seed(seed)
    np.random.seed(seed)
    dump_folder = '../../stage2_processed_data/KPTimes-lr-2000/{}/v6_bt_aug/'.format(seed)
    Path(dump_folder).mkdir(parents=True, exist_ok=True)

    process_data('KPTimes-lr-2000_train.jsonl'   , dump_folder, seed)
    process_data('KPTimes-lr-2000_dev_KPTimes-lr-2000.jsonl' , dump_folder, seed)
    process_data('KPTimes-lr-2000_test_KPTimes-lr-2000.jsonl', dump_folder, seed)
    shutil.copy('../../processed_data/{}/aug_v6/KPTimes-lr-2000_metadata.pkl'.format(seed), dump_folder)  # Copy metadata file as it is