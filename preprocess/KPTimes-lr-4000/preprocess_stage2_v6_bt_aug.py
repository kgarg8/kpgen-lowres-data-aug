...
def process_data(file, path, seed):
    ...
    for i in tqdm(range(len(data))):
        ...
        if 'train' in file and i%2 == 1:
            ...
            except: # added for KPTimes where some articles are too short and do not have body
                continue
...
for seed in seeds:
    random.seed(seed)
    np.random.seed(seed)
    dump_folder = '../../stage2_processed_data/KPTimes-lr-4000/{}/v6_bt_aug/'.format(seed)
    Path(dump_folder).mkdir(parents=True, exist_ok=True)

    process_data('KPTimes-lr-4000_train.jsonl'   , dump_folder, seed)
    process_data('KPTimes-lr-4000_dev_KPTimes-lr-4000.jsonl' , dump_folder, seed)
    process_data('KPTimes-lr-4000_test_KPTimes-lr-4000.jsonl', dump_folder, seed)
    shutil.copy('../../processed_data/{}/aug_v6/KPTimes-lr-4000_metadata.pkl'.format(seed), dump_folder)  # Copy metadata file as it is